...
namespace emotitron.NST
{
	...
	public static class UnetBitstreamSerializers
	{
		...
		private static NetworkWriter reusablewriter = new NetworkWriter();
		...
		public static void SendPayloadArrayToAllClients(this NetworkWriter writer, short msgType, int channel = Channels.DefaultReliable)
		{
            //reusablewriter.StartMessage(msgType); //UNet Code
            //byte[] writerArray = writer.AsArray(); //UNet Code
            ...
            //reusablewriter.FinishMessage(); //UNet Code

            byte[] messageBytes = new byte[reusablewriter.ToArray().Length];
            Buffer.BlockCopy(reusablewriter.ToArray(), 0, messageBytes, 0, reusablewriter.ToArray().Length);
            BytesMessage msg = new BytesMessage(messageBytes);
            ...
            foreach (NetworkConnection nc in NetworkServer.connections.Values)
			{
				...
				if (nc.isReady)
				{
                    //nc.SendWriter(reusablewriter, channel); //UNet Code
                    nc.Send(msgType, msg, channel);
                }
			}
		}
		...
		public static void SendBitstreamToThisConn(this NetworkConnection conn, ref UdpBitStream bitstream, int channel = Channels.DefaultReliable)
		{
            //reusablewriter.StartMessage(HeaderSettings.single.masterMsgTypeId); //UNet Code
            //reusablewriter.WriteUncountedByteArray(bitstream.Data, bitstream.BytesUsed);
            //reusablewriter.FinishMessage();
            //conn.SendWriter(reusablewriter, channel); //UNet Code
            byte[] messageBytes = new byte[bitstream.BytesUsed];
            Buffer.BlockCopy(bitstream.Data, 0, messageBytes, 0, bitstream.BytesUsed);
            BytesMessage msg = new BytesMessage(messageBytes);
            conn.Send((short)MsgType.DefaultMsgTypeId, msg, channel);
        }
