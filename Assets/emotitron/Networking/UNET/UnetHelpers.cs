...
#pragma warning disable CS0618 // UNET is obsolete
  ...
  //          //unetwriter.StartMessage(NetworkServer.active ? SVR_TO_CLIENT : CLIENT_TO_SVR); //UNet Code
  //          //unetwriter.Write(NetworkServer.active ? SVR_TO_CLIENT : CLIENT_TO_SVR);

  //          /// There is a built in overload for working with UNET
  //          unetwriter.Write(bitstream);
  ...
  //          //unetwriter.FinishMessage(); //UNet Code

  //          /// Server send to all. Owner client send to server.
  //          if (NetworkServer.active)
  //          {
  //              //NetworkServer.SendWriterToReady(null, unetwriter, Channels.DefaultUnreliable); //UNet Code
  //              BytesMessage msg = new BytesMessage();
  //              msg.Serialize(unetwriter);
  //              NetworkServer.SendToReady(null, (short)MsgType.SVR_TO_CLIENT, msg, Channels.DefaultUnreliable);
  //          }
  //          else
  //          {
  //              //ClientScene.readyConnection.SendWriter(unetwriter, Channels.DefaultUnreliable); //UNet Code
  //              BytesMessage msg = new BytesMessage();
  //              msg.Serialize(unetwriter);
  //              ClientScene.readyConnection.Send((short)MsgType.CLIENT_TO_SVR, msg, Channels.DefaultUnreliable);
  //          }
  ...
  //          //unetwriter.Write(NetworkServer.active ? SVR_TO_CLIENT : CLIENT_TO_SVR);
		...
		//	/// There is a built in overload for working with UNET
		//	//unetwriter.Write(buffer, 0, bytecount);
  ...
  //          {
  //              //NetworkServer.SendWriterToReady(null, unetwriter, Channels.DefaultUnreliable); //UNet Code
  //              BytesMessage msg = new BytesMessage(buffer);
  //              //msg.Serialize(unetwriter);
  //              NetworkServer.SendToReady(null, (short)MsgType.SVR_TO_CLIENT, msg, Channels.DefaultUnreliable);
  //          }
  //          else
  ...
  //          }
		...
		//	for (uint i = (uint)netMsg.reader.Position; i < netMsg.reader.Length; ++i)
