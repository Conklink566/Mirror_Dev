...
namespace Smooth
{
    ...
    public class SmoothSync : NetworkBehaviour
    {
        ...
        void FixedUpdate()
        {
            if(this.gameObject.name == "PhysicsObject")
            {
                print("check 1");
            }
        ...
        void sendState()
        {
            ...
            else
            {
                ...
                {
                    //NetworkManager.singleton.client.connection.SendByChannel(MsgType.SmoothSyncFromOwnerToServer, sendingTempState, networkChannel); //Unity UNet Code
                    NetworkManager.singleton.client.connection.Send((short)Mirror.MsgType.SmoothSyncFromOwnerToServer, sendingTempState); //Mirror UNet Code
                }
                else
                { 
                    //NetworkClient.allClients[0].connection.SendByChannel(MsgType.SmoothSyncFromOwnerToServer, sendingTempState, networkChannel); //Unity UNet Code
                    NetworkClient.allClients[0].connection.Send((short)Mirror.MsgType.SmoothSyncFromOwnerToServer, sendingTempState); //Mirror UNet Code
                }
            }
        }
        ...
        public override void OnStartServer()
        {
            if (GetComponent<NetworkIdentity>().localPlayerAuthority)
            {
                ...
                //}
                if (!NetworkServer.handlers.ContainsKey((short)Mirror.MsgType.SmoothSyncFromOwnerToServer))
                {
                    NetworkServer.RegisterHandler((short)Mirror.MsgType.SmoothSyncFromOwnerToServer, HandleSyncFromOwnerToServer);
                }
                if (NetworkClient.allClients.Count != 0)
                {
                    if (NetworkManager.singleton)
                    {
                        ...
                        //}
                        if (!NetworkManager.singleton.client.handlers.ContainsKey((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners))
                        {
                            NetworkManager.singleton.client.RegisterHandler((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners, HandleSyncFromServerToNonOwners);
                        }
                    }
                    else
                    {
                        ...
                        //}
                        if (!NetworkClient.allClients[0].handlers.ContainsKey((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners))
                        {
                            NetworkClient.allClients[0].RegisterHandler((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners, HandleSyncFromServerToNonOwners);
                        }
                    }
                }
            }
            ...
            if (!NetworkServer.active)
            {
                if (NetworkManager.singleton)
                {
                    if (NetworkManager.singleton.client != null &&
                        !NetworkManager.singleton.client.handlers.ContainsKey((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners))
                    {
                        NetworkManager.singleton.client.RegisterHandler((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners, HandleSyncFromServerToNonOwners);
                    }
                }
                else
                {
                    if (NetworkClient.allClients != null && 
                        ...
                        !NetworkClient.allClients[0].handlers.ContainsKey((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners))
                    {
                        NetworkClient.allClients[0].RegisterHandler((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners, HandleSyncFromServerToNonOwners);
                    }
                }
...
#region Sync Properties
        ...
        void SendStateToNonOwners(MessageBase state)
        {
            ...
            foreach(NetworkConnection conn in NetworkServer.connections.Values)
            {
                ...
                if (conn != null && conn != netID.clientAuthorityOwner && conn.hostId != -1 && conn.isReady)
                {
                    ...
                    //conn.SendByChannel(MsgType.SmoothSyncFromServerToNonOwners, state, networkChannel); //Unity Unet Code
                    conn.Send((short)Mirror.MsgType.SmoothSyncFromServerToNonOwners, state); //Mirror UNet Code
                }
            }
        }
    ...
    bool isObservedByConnection(NetworkConnection conn)
        {
            foreach(NetworkConnection observer in netID.observers.Values)
            {
                if (observer == conn)
                    return true;
            }
        ...
        public float GetNetworkSendInterval() //Removed override, since Mirror removed this from Unity UNet
        {
            if (sendRate == 0) return 0;
            return 1 / sendRate;
        }

        public int GetNetworkChannel() //Removed override, since Mirror removed this from Unity UNet
        {
