...
namespace Smooth
{
    ...
    public class NetworkState : MessageBase
    {
        ...
        override public void Deserialize(NetworkReader reader)
        {
            ...
            //NetworkInstanceId netID = reader.ReadNetworkId(); //Unity UNet Code
            NetworkIdentity netID = reader.ReadNetworkIdentity(); //Mirror UNet Code
            ...
            if (NetworkServer.active)
            {
                //ob = NetworkServer.FindLocalObject(netID); //Unity UNet Code
                ob = NetworkIdentity.spawned[netID.netId].gameObject; //Mirror UNet Code
            }
            else
            {
                //ob = ClientScene.FindLocalObject(netID); //Unity UNet Code
                ob = NetworkIdentity.spawned[netID.netId].gameObject; //Mirror UNet Code
            }
